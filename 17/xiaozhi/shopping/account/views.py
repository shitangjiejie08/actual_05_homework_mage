            ...
            try:
                with transaction.atomic():
                    ...
                    user_ext = UserExt.objects.create(user=user, realname='', birthday=datetime.date(1945, 1, 1),\
                                                      ...
                                                      validkey=validkey)
                    content = '欢迎注册[小智的商城], 此激活时间的有效期为10分钟，请在此时间内激活,请点击此处进行激活用户: http://192.168.10.220:8888/account/active/?username=' \
        ...
        try:
            ...
            if user.userext.status == 0 and user.userext.validkey != '':
                if user.userext.validkey == validkey:
                    ...
                    if time.time()() - user.userext.expiration < 600:
                            user.userext.save()
                            messages.add_message(request, messages.INFO, '激活成功, 请登录')
                    else:
                        messages.add_message(request, messages.ERROR, '激活失败, 已经过了激活有效期')
                else:
    ...
    def form_valid(self, form):
        try:
            with transaction.atomic():
                ...
                user.userext.expiration = time.time()
                user.userext.save()
                content = '欢迎注册[小智的商城], 此激活时间的有效期为10分钟，请在此时间内重置,请点击此处进行重置用户: http://192.168.10.220:8888/account/reset_password_confirm/?username=' \
...
class ResetPasswordConfirmView(FormView):
    ...
    def form_valid(self, form):
        ...
        if time.time() - user.userext.expiration < 600:
            ...
            messages.add_message(self.request, messages.ERROR, '激活失败, 已经过了激活有效期')
            return render(self.request, self.template_name, {'form': form})
